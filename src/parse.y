...
%token <nd>  tSTRING tSTRING_PART tSTRING_MID tSTRING_SP tHD_STRING_SP
...
heredoc_string_interp : tHD_STRING_MID
        		...
        		local_nest(p);
        	      ...
        	      if (is_strterm_type(p, STR_FUNC_PROC)) {
        		if ((int)(intptr_t)($1)->cdr->cdr == 0)
        		  inf->doc = push(inf->doc, new_block(p, 0, $3));
        		else
        		  inf->doc = push(push(inf->doc, $1), new_block(p, 0, $3));
        		local_unnest(p);
        	      }
        	      else
...
parse_string(parser_state *p)
{
  ...
  while ((c = nextc(p)) != end || nest_level != 0) {
    ...
    if (ISSPACE(c)) {
      if (type & STR_FUNC_LAZYEE_SP) {
        int save_toklen = p->bidx;
        if ((p->column == 1) && (save_toklen == 0) && (c != '\n')) {
          while (((c = nextc(p)) != -1) && (c != '\n') && ISSPACE(c))
            ;
          pushback(p, c);
          continue;
        ...
        do {
          if (c == '\n') {
            ...
            heredoc_treat_nextline(p);
            if (p->parsing_heredoc != NULL) {
              return tHD_STRING_SP;
            }
            return tSTRING_SP;
          }
          tokadd(p, c);
          ;
        } while (((c = nextc(p)) != -1) && ISSPACE(c));
        pushback(p, c);
        continue;
      }
...
#define no_digits() do {yyerror(p,"numeric literal without digits"); return 0;} while (0)
  ...
  case '%':
    if (IS_BEG()) {
      ...
      case 'P':
        p->lex_strterm = new_strterm(p, str_lazyEE, term, paren);
        return tWORDS_BEG;

      case 'p':
        p->lex_strterm = new_strterm(p, str_lazyEE_sp, term, paren);
        return tWORDS_BEG;
